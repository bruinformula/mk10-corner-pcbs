...
uint32_t ms_since_ttemp_read;
uint32_t ms_since_btemp_read;
...
void CANMailman(void) {
	...
	if (HAL_GetTick() - ms_since_ttemp_broadcast > TIRE_TEMP_TRANSMISSION_PERIOD) {
		...
		txstatus = CANTransmitMinion (&hcan1, &CTXHeader, ttemp_dataframes[4].array);
		if (txstatus != HAL_OK)
...
void spamCan(void) {
	if(HAL_GetTick() - lastCanSpam >= 100 ){ //scuffed but itll work for testing
		...
		//
		uint32_t mailbox = 0;
		for (int i = 0; i < 3 * CAN_RETRY_LIMIT; i ++) {
